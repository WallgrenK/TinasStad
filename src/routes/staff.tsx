...
function StaffMobilePage() {
  ...
  const myJobs = useMemo(
    () =>
      jobsForStaff(ME_ID, TODAY_DATE).sort((a, b) =>
        a.startTime.localeCompare(b.startTime),
      ),
    [],
  )

  const [tab, setTab] = useState<'today' | 'schedule' | 'time' | 'profile'>(
    'today',
  )
  ...
  const open = openJobId
    ? (myJobs.find((j) => j.id === openJobId) ?? null)
    : null
  ...
  return (
    <div className="admin-scope min-h-screen bg-[var(--color-admin-surface)]">
      ...
      <JobSheet
        ...
        onStatusChange={(s) =>
          open && setStatuses((p) => ({ ...p, [open.id]: s }))
        }
...
function ComingSoonPanel({ tab }: { tab: 'schedule' | 'time' | 'profile' }) {
  const map = {
    schedule: {
      title: 'Veckoschema',
      body: 'Här kommer du snart kunna se hela veckans pass.',
    },
    time: {
      title: 'Tidrapporter',
      body: 'Veckosammanställning av din arbetade tid.',
    },
    profile: {
      title: 'Profil',
      body: 'Inställningar för konto och aviseringar.',
    },
  } as const
  const m = map[tab]
  return (
    <div className="flex flex-1 flex-col items-center justify-center px-6 py-16 text-center">
      ...
      <p className="mt-1 max-w-xs text-sm text-[var(--color-admin-muted)]">
        {m.body}
      </p>
      ...
      >
        <div className="space-y-5 p-5">
          <SheetHeader className="space-y-1 p-0 text-left">
            ...
            <SheetDescription>
              {job.service} · {job.startTime}–{job.endTime}
            </SheetDescription>
          </SheetHeader>
          ...
          <div className="space-y-2 text-sm">
            <a
              ...
              className="flex items-center gap-2 rounded-md border border-[var(--color-admin-border)] bg-white p-3"
            >
                  ...
                  >
                    ...
                    <span
                      className={
                          ...
                          ? 'line-through text-[var(--color-admin-muted)]'
                          : ''
                      }
                    >
                      {item}
              ...
              <dl className="mt-3 grid grid-cols-3 gap-2 text-xs">
                ...
                <div>
                  ...
                  <dd className="text-sm font-semibold tabular-nums">
                    ...
                    {formatDuration(
                      Math.abs(actualMin - job.estimatedDurationMin),
                    )}
                  </dd>
                </div>
              </dl>
