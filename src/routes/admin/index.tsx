          ...
          <div className="grid grid-cols-2 gap-2">
            {quickActions.map((a) => (
              <button
                ...
                className="group flex flex-col items-start gap-3 rounded-lg border border-var(--color-admin-border) bg-(--color-admin-bg) p-3 text-left transition-colors hover:border-(--color-admin-accent) hover:bg-(--color-admin-accent-soft)"
              >
                <a.icon className="h-4 w-4 text-var(--color-admin-accent)" />
                <span className="text-[13px] font-medium leading-tight text-var(--color-admin-text)">
                  {a.label}
                </span>
              </button>
            ))}
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <section className="admin-card overflow-hidden">
          <header className="flex items-center justify-between border-b border-var(--color-admin-border) px-5 py-3">
            ...
            <Link
              to="/admin/bookings"
              className="flex items-center gap-1 text-xs text-var(--color-admin-accent) hover:underline"
            >
              Alla bokningar <ArrowRight className="h-3 w-3" />
            </Link>
          </header>
          {todaysBookings.length === 0 ? (
            <div className="px-5 py-10 text-center text-sm text-var(--color-admin-muted)">
              Inga bokningar idag.
            </div>
          ) : (
            <ul className="divide-y divide-var(--color-admin-border)">
              {todaysBookings.map((b) => (
                <li key={b.id} className="flex items-center gap-4 px-5 py-3">
                  <div className="w-14 text-sm font-medium tabular-nums text-var(--color-admin-text)">
                    {formatTime(b.startAt)}
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="truncate text-sm font-medium text-var(--color-admin-text)">
                      {b.customerName}
                    </div>
                    <div className="truncate text-xs text-var(--color-admin-muted)">
                      {b.service} · {b.address}
                    </div>
                  </div>
