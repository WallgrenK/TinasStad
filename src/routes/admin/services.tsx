...
export const Route = createFileRoute("/admin/services")({
  head: () => ({ meta: [{ title: "Tjänster · Admin · Tinas Städ" }, { name: "robots", content: "noindex" }] }),
...
function ServicesPage() {
  const [services, setServices] = useState<AdminService[]>(() => [...adminServices].sort((a, b) => a.order - b.order));
  ...
  const filtered = useMemo(() => {
    ...
    return services.filter((s) => s.title.toLowerCase().includes(q) || s.tagline.toLowerCase().includes(q));
  }, [services, query]);
  ...
  const togglePublished = (id: string) => {
    setServices((prev) =>
      prev.map((s) => (s.id === id ? { ...s, published: !s.published, updatedAt: new Date().toISOString() } : s)),
    );
    toast.success("Status uppdaterad (lokalt)");
  };

  const updateField = <K extends keyof AdminService>(id: string, key: K, value: AdminService[K]) => {
    setServices((prev) => prev.map((s) => (s.id === id ? { ...s, [key]: value, updatedAt: new Date().toISOString() } : s)));
  };
  ...
  return (
    <div>
      ...
      <div className="mb-4 relative max-w-md">
        ...
        <Input placeholder="Sök tjänst…" value={query} onChange={(e) => setQuery(e.target.value)} className="h-9 pl-9" />
      </div>
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className="truncate text-sm font-medium text-[var(--color-admin-text)]">{s.title}</span>
                    {s.rutEligible && (
                  ...
                  <div className="mt-0.5 truncate text-[13px] text-[var(--color-admin-muted)]">{s.tagline}</div>
                </div>
            ...
            <>
              ...
              <div className="mt-6 space-y-6">
                <div className="grid gap-4 sm:grid-cols-2">
                  <div>
                    <Label className="mb-1.5 block text-[11px] uppercase tracking-[0.1em] text-[var(--color-admin-muted)]">Titel</Label>
                    <Input value={open.title} onChange={(e) => updateField(open.id, "title", e.target.value)} />
                  </div>
                  <div>
                    <Label className="mb-1.5 block text-[11px] uppercase tracking-[0.1em] text-[var(--color-admin-muted)]">Slug</Label>
                    <Input value={open.slug} onChange={(e) => updateField(open.id, "slug", e.target.value)} />
                  </div>
                </div>

                <div>
                  <Label className="mb-1.5 block text-[11px] uppercase tracking-[0.1em] text-[var(--color-admin-muted)]">Kort beskrivning</Label>
                  <Textarea rows={3} value={open.tagline} onChange={(e) => updateField(open.id, "tagline", e.target.value)} />
                </div>

                <div className="grid gap-4 sm:grid-cols-3">
                  <div>
                    <Label className="mb-1.5 block text-[11px] uppercase tracking-[0.1em] text-[var(--color-admin-muted)]">Pris fr.</Label>
                    <Input
                      type="number"
                      value={open.priceFromSek}
                      onChange={(e) => updateField(open.id, "priceFromSek", Number(e.target.value))}
                    />
                  </div>
                  <div>
                    <Label className="mb-1.5 block text-[11px] uppercase tracking-[0.1em] text-[var(--color-admin-muted)]">Enhet</Label>
                    <Select value={open.unit} onValueChange={(v) => updateField(open.id, "unit", v as AdminService["unit"])}>
                      <SelectTrigger><SelectValue /></SelectTrigger>
                      <SelectContent>
                        <SelectItem value="tim">kr / timme</SelectItem>
                        <SelectItem value="tillfälle">kr / tillfälle</SelectItem>
                        <SelectItem value="kvm">kr / kvm</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  <div>
                    <Label className="mb-1.5 block text-[11px] uppercase tracking-[0.1em] text-[var(--color-admin-muted)]">Standardlängd (min)</Label>
                    <Input
                      type="number"
                      value={open.durationMin}
                      onChange={(e) => updateField(open.id, "durationMin", Number(e.target.value))}
                    />
                  </div>
                </div>

                <div>
                  <Label className="mb-2 block text-[11px] uppercase tracking-[0.1em] text-[var(--color-admin-muted)]">Det här ingår</Label>
                  <div className="space-y-2">
                    ...
                    <button
                      onClick={() => updateField(open.id, "includes", [...open.includes, ""])}
                ...
                <div className="flex items-center justify-between rounded-lg border border-[var(--color-admin-border)] bg-[var(--color-admin-surface)] px-4 py-3">
                  <div>
                    <div className="text-sm font-medium text-[var(--color-admin-text)]">RUT-berättigad</div>
                    <div className="text-[12px] text-[var(--color-admin-muted)]">Visas med RUT-märkning på webbplatsen.</div>
                  </div>
                  <Switch checked={open.rutEligible} onCheckedChange={(v) => updateField(open.id, "rutEligible", v)} />
                </div>

                <div className="flex items-center justify-between rounded-lg border border-[var(--color-admin-border)] bg-[var(--color-admin-surface)] px-4 py-3">
                  <div>
                    <div className="text-sm font-medium text-[var(--color-admin-text)]">Publicerad</div>
                    <div className="text-[12px] text-[var(--color-admin-muted)]">Synlig på /tjanster och i offertformuläret.</div>
                  </div>
                  <Switch checked={open.published} onCheckedChange={(v) => updateField(open.id, "published", v)} />
                </div>
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Formuläret aktiveras när backend kopplas in.</DialogDescription>
          </DialogHeader>
        </DialogContent>
      </Dialog>
