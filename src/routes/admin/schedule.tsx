...
function SchedulePage() {
  ...
  const visibleStaff = staffFilter === "all" ? allStaff.filter((s) => s.status === "active") : allStaff.filter((s) => s.id === staffFilter);

  const filteredJobs = useMemo(() => {
    return jobs.filter((j) => {
      if (serviceFilter !== "all" && j.service !== serviceFilter) return false;
      if (staffFilter !== "all" && !j.assignedStaffIds.includes(staffFilter)) return false;
      return true;
    });
    ...
    <div>
      ...
      <div className="admin-card mb-4 flex flex-wrap items-center gap-2 p-3">
        <div className="flex items-center gap-1">
          <button onClick={() => shift(-1)} className="grid h-8 w-8 place-items-center rounded-md border border-[var(--color-admin-border)] hover:bg-[var(--color-admin-surface)]">
            <ChevronLeft className="h-4 w-4" />
          </button>
          <div className="min-w-[180px] px-3 text-sm font-medium tabular-nums">
            {view === "day"
              ? formatSv(date, { weekday: "long", day: "numeric", month: "long" })
        ...
        <div className="ml-auto flex flex-wrap items-center gap-2">
          ...
          <Select value={serviceFilter} onValueChange={setServiceFilter}>
            <SelectTrigger className="h-9 w-[180px]"><SelectValue placeholder="Tjänst" /></SelectTrigger>
            <SelectContent>
      ...
      {view === "day" ? (
        <ScheduleTimeline staff={visibleStaff} jobs={dayJobs} onJobClick={(j) => setOpenJobId(j.id)} />
      ) : (
        <ScheduleWeekGrid weekStart={weekStart} jobs={weekJobs} onJobClick={(j) => setOpenJobId(j.id)} />
      )}
          ...
          {open && (
            <>
              <SheetHeader>
                ...
                <SheetDescription>
                  {open.service} · {formatSv(open.scheduledDate)} · {open.startTime}–{open.endTime}
                </SheetDescription>
              </SheetHeader>
              ...
              <div className="mt-6 space-y-6">
                ...
                <section className="space-y-2 text-sm">
                  ...
                  <div className="flex items-center gap-2 text-[var(--color-admin-muted)]">
                    <Phone className="h-4 w-4" /> <span className="tabular-nums">{open.customerPhone}</span>
                  </div>
                </section>
                ...
                <section>
                  ...
                  <div className="mb-3 flex items-center gap-2">
                    {openStaff.map((s) => (
                      <div key={s.id} className="flex items-center gap-2 rounded-full border border-[var(--color-admin-border)] bg-white py-1 pl-1 pr-3">
                  ...
                  <Select defaultValue={open.assignedStaffIds[0]}>
                    <SelectTrigger className="h-9"><SelectValue placeholder="Lägg till personal…" /></SelectTrigger>
                    <SelectContent>
                      {allStaff.map((s) => (
                        <SelectItem key={s.id} value={s.id}>{s.name}</SelectItem>
                      ))}
