...
function SettingsPage() {
  ...
  const save = () => {
    ...
    toast.success('Ändringar sparade', {
      description: 'Mock-data — backend kopplas i nästa iteration.',
    })
  }
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="company" className="w-full">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <Field label="Organisationsnummer">
                  <Input
                    value={company.orgNr}
                    onChange={(e) => {
                      setCompany({ ...company, orgNr: e.target.value })
                      markDirty()
                    }}
                  />
                </Field>
        ...
        <TabsContent value="hours" className="mt-6">
          <SectionCard
            title="Öppettider"
            description="När kunder kan nå er via telefon och e-post."
          >
            <div className="divide-y divide-[var(--color-admin-border)]">
              {WEEKDAYS.map((day) => {
                ...
                return (
                  <div
                    key={day.key}
                    className="grid grid-cols-1 sm:grid-cols-[140px_1fr_auto] items-center gap-3 py-3"
                  >
                    <div className="font-medium text-[var(--color-admin-text)] text-sm">
                      {day.label}
                    </div>
                    <div className="flex items-center gap-2">
                      <Input
                        ...
                        className="w-28"
                      />
                      <span className="text-[var(--color-admin-muted)] text-sm">
                        –
                      </span>
                      <Input
                        type="time"
        ...
        <TabsContent value="seo" className="mt-6">
          <div className="grid gap-6 lg:grid-cols-3">
            <SectionCard
              ...
              description="Används som fallback för sidor utan egen metadata."
            >
              <div className="space-y-4">
                <Field
                  label="Meta-titel"
                  hint={`${seo.title.length}/60 tecken`}
                >
                  <Input
                    value={seo.title}
                    onChange={(e) => {
                ...
                <Field
                  ...
                  hint={`${seo.description.length}/160 tecken`}
                >
                  <Textarea
            ...
            <SectionCard
              title="Google-förhandsvisning"
              description="Ungefärlig visning i sökresultat."
            >
              <div className="rounded-lg border border-[var(--color-admin-border)] bg-white p-4">
                ...
                <div className="text-[18px] leading-tight text-[#1a0dab] mt-1 line-clamp-2">
                  {seo.title || 'Sidans titel'}
                </div>
                <div className="text-sm text-[#4d5156] mt-1 line-clamp-3">
                  {seo.description || 'Sidans beskrivning visas här.'}
                </div>
              </div>
            </SectionCard>
          </div>
        ...
        <TabsContent value="notifications" className="mt-6">
          <SectionCard
            title="E-postnotiser"
            description="Vad ska skickas till kontakt@tinasstad.se."
          >
            <div className="divide-y divide-[var(--color-admin-border)]">
              <ToggleRow
                ...
                onChange={(v) => {
                  setNotifications({ ...notifications, newLead: v })
                  markDirty()
                }}
              />
              <ToggleRow
                ...
                onChange={(v) => {
                  setNotifications({ ...notifications, newBooking: v })
                  markDirty()
                }}
              />
              <ToggleRow
                ...
                onChange={(v) => {
                  setNotifications({ ...notifications, cancellation: v })
                  markDirty()
                }}
              />
              <ToggleRow
                ...
                onChange={(v) => {
                  setNotifications({ ...notifications, review: v })
                  markDirty()
                }}
              />
              <ToggleRow
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <div className="font-medium text-[var(--color-admin-text)]">
                        {it.name}
                      </div>
                      {connected ? (
            ...
            <div className="divide-y divide-[var(--color-admin-border)]">
              {TEAM.map((m) => (
                <div key={m.email} className="flex items-center gap-3 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium text-[var(--color-admin-text)] truncate">
                      {m.name}
                    </div>
                    <div className="text-xs text-[var(--color-admin-muted)] truncate">
                      {m.email}
                    </div>
                  </div>
...
function SectionCard({
  ...
  return (
    <section
      className={`rounded-xl border border-[var(--color-admin-border)] bg-white p-5 sm:p-6 ${className}`}
    >
      <header className="flex items-start justify-between gap-3 mb-5">
        <div>
          ...
          {description && (
            <p className="text-sm text-[var(--color-admin-muted)] mt-0.5">
              {description}
            </p>
          )}
        </div>
  ...
  return (
    <div className="flex items-start justify-between gap-4 py-4">
      <div className="min-w-0">
        ...
        <div className="text-sm text-[var(--color-admin-muted)] mt-0.5">
          {description}
        </div>
      </div>
