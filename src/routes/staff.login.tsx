    ...
    <div className="admin-scope flex min-h-screen flex-col bg-[var(--color-admin-surface)]">
      <div className="mx-auto flex w-full max-w-md flex-1 flex-col px-6 pb-10 pt-10">
        ...
        <div className="mt-10 rounded-2xl border border-[var(--color-admin-border)] bg-white p-6 shadow-sm">
          <div className="mb-6">
            <h1 className="text-2xl font-semibold tracking-tight text-[var(--color-admin-text)]">Logga in</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              <label htmlFor="phone" className="text-sm font-medium text-[var(--color-admin-text)]">
                Telefonnummer
              </label>
              <Input
                id="phone"
            ...
            <div className="space-y-1.5">
              <label htmlFor="pin" className="text-sm font-medium text-[var(--color-admin-text)]">
                PIN-kod
              </label>
              <Input
                id="pin"
            ...
            <Button type="submit" disabled={loading} className="h-12 w-full text-base">
              {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : <>Logga in <ArrowRight className="ml-1 h-4 w-4" /></>}
            </Button>

            <button type="button" className="block w-full text-center text-sm text-[var(--color-admin-muted)] hover:underline">
          ...
          <div className="grid grid-cols-2 gap-2">
            {staff.slice(0, 4).map((s) => (
              ...
              >
                ...
                <span className="truncate text-[var(--color-admin-text)]">{s.name.split(" ")[0]}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <p className="mt-auto pt-10 text-center text-xs text-[var(--color-admin-muted)]">
          Är du admin?{" "}
          <Link to="/admin/login" className="font-medium text-[var(--color-admin-text)] hover:underline">
            Logga in här
          </Link>
        </p>
      </div>
